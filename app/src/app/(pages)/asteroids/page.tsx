...
import { AlertTriangle, Radar, ShieldCheck, ArrowRight, Activity, Target } from "lucide-react";
...
export default function AsteroidsPage() {
  ...
  const { data: asteroids, isLoading, isFetching, error } = useQuery({
    ...
    <div className="min-h-screen pt-12 pb-24 px-4 sm:px-8 flex flex-col items-center">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-serif text-2xl md:text-3xl font-bold tracking-wider">Monitoramento NEO</h1>
            <p className="text-sm text-muted-foreground">Objetos Próximos à Terra (NeoWs)</p>
          </div>
        </div>
      ...
      {error && (
        <div className="w-full max-w-xl p-8 border border-destructive/30 bg-destructive/10 rounded-3xl text-destructive text-center">
          ...
          <p>Não foi possível estabelecer conexão com a rede de monitoramento.</p>
        </div>
      )}
        ...
        <div className="w-full max-w-6xl">
          ...
          >
            {asteroids.map((asteroid) => {
              ...
              const velocity = parseFloat(asteroid.close_approach_data[0]?.relative_velocity.kilometers_per_hour || "0").toLocaleString('pt-BR', { maximumFractionDigits: 0 });
              const distance = parseFloat(asteroid.close_approach_data[0]?.miss_distance.kilometers || "0").toLocaleString('pt-BR', { maximumFractionDigits: 0 });
              ...
              return (
                ...
                >
                  {/* Fundo Decorativo */}
                  <div className={`absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl opacity-20 pointer-events-none ${isHazardous ? 'bg-destructive' : 'bg-primary'}`} />
                  ...
                  <div className="flex justify-between items-start mb-6 relative z-10">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-mono mt-1">ID: {asteroid.id}</p>
                    </div>
                    {isHazardous ? (
                  ...
                  <div className="space-y-4 relative z-10">
                    <div className="flex justify-between items-end border-b border-white/5 pb-2">
                      <span className="text-xs text-muted-foreground uppercase tracking-wider">Tamanho Máx.</span>
                      <span className="text-lg font-semibold text-foreground flex items-center gap-1">
                        {size} <span className="text-xs text-muted-foreground">metros</span>
                      </span>
                    </div>
                    
                    <div className="flex justify-between items-end border-b border-white/5 pb-2">
                      <span className="text-xs text-muted-foreground uppercase tracking-wider">Velocidade</span>
                      <span className="text-sm font-mono text-foreground flex items-center gap-1">
                        {velocity} <span className="text-xs text-muted-foreground">km/h</span>
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-sm font-mono text-foreground flex items-center gap-1">
                        {distance} <span className="text-xs text-muted-foreground">km</span>
                      </span>
                    </div>
                  </div>
                </motion.div>
